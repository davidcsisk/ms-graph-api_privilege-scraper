...
# Broader queries to find markdown files and code mentioning permissions
queries = [
    "filename:permissions extension:md",
    "permissions- extension:md",
    '"permissions-" extension:md',
    "permissions in:file extension:md",
    '"Permission" path:api-reference',
    "permissions repo:microsoftgraph",
]
...
for q in queries:
    ...
    for item in data.get("items", []):
        ...
        seen[key] = True
        print(f"- {key}")
        # try to fetch contents metadata to get download_url
        try:
            ...
            if cr.status_code == 200:
                ...
                print(f"  contents metadata fetch failed: {cr.status_code}")
        except Exception as e:
